...
class DinoAudioEngine {
  ...
  // Bicara Kata Mandarin Standar (TTS)
  speakMandarin(text, rate = 0.85) {
    ...
    try {
      ...
      utterance.rate = rate; // Tempo standar
  ...
  // Bicara Cerita & Kalimat Mandarin Tempo Lambat (Slow & Clear Voice untuk Siswa SD)
  speakMandarinSlow(text, onEndCallback = null) {
    if (!text || !this.voiceEnabled || !('speechSynthesis' in window)) {
    ...
    try {
      ...
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = 'zh-CN';
      utterance.rate = 0.65; // Kecepatan lamban agar jelas didengar anak-anak
      ...
      console.warn('Speech slow notice:', err);
      if (typeof onEndCallback === 'function') onEndCallback();
    }
  }
