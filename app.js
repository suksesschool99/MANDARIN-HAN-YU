...
window.refreshTeacherTaskLink = function(isBookChanged) {
  ...
  const story = (window.DINO_DATA && window.DINO_DATA.stories && window.DINO_DATA.stories.find(s => s.book === bId)) || null;
  ...
  if (mod === 'stroke') {
    ...
    if (taskInput) {
      taskInput.value = `Membaca Cerita Han Yu ${book}${story ? ' (' + story.title + ')' : ''}`;
    }
  } else if (mod === 'match') {
    if (bookGrp) bookGrp.style.display = 'block';
    if (unitGrp) unitGrp.style.display = 'none';
  ...
  // Format target URL to tugas.html
  let baseUrl = window.location.href.split('?')[0];
  if (baseUrl.endsWith('/index.html')) {
    baseUrl = baseUrl.replace(/\/index\.html$/, '/tugas.html');
  } else if (baseUrl.endsWith('/')) {
    baseUrl = baseUrl + 'tugas.html';
  } else if (!baseUrl.endsWith('/tugas.html')) {
    baseUrl = baseUrl.substring(0, baseUrl.lastIndexOf('/') + 1) + 'tugas.html';
  }
  ...
  if (linkHref) {
    ...
    linkHref.textContent = '🌐 Buka Halaman Murid (tugas.html): ' + fullUrl;
  }
  ...
  if (pMod) {
    if (mod === 'stroke') pMod.textContent = '✍️ Menulis Guratan Hanzi';
    else if (mod === 'story') pMod.textContent = '📖 Membaca Cerita (Tempo Lambat)';
    else if (mod === 'match') pMod.textContent = '🧩 Game Mencocokkan Kartu';
    else pMod.textContent = '📝 Kuis Jumlah Guratan Ber-Audio';
  }
  if (pBook) pBook.textContent = `Han Yu ${book} (汉语 第${hanziNum}册)`;
  if (pUnit) pUnit.textContent = mod === 'stroke' ? `Unit ${unit}${uName ? ' (' + uName + ')' : ''}` : (mod === 'story' ? (story ? story.title + ' • ' + story.meaning : '-') : '-');
  if (pTarget) {
    if (mod === 'stroke') pTarget.textContent = `${target} Kali Tulis sampai Telur Menetas 🥚➔🦖`;
    else if (mod === 'story') pTarget.textContent = 'Dengarkan Audio Suara Lamban 🐢🗣️';
    else if (mod === 'match') pTarget.textContent = `${target} Pasang Kartu Dino`;
    else pTarget.textContent = '15 Soal Pilihan Berganda (A, B, C, D)';
  }
};
  ...
  if (linkOutput && linkOutput.value) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(linkOutput.value).then(() => {
        if (btnCopy) btnCopy.innerHTML = '✅ Link Halaman Murid Tersalin!';
        if (window.dinoAudio) window.dinoAudio.playSfx('correct');
        setTimeout(() => { if (btnCopy) btnCopy.innerHTML = '<span>📋</span> Salin Link Halaman Murid'; }, 2000);
      }).catch(() => {
...
window.shareTeacherTaskWA = function() {
  ...
  const message = `🦖 *DINO MANDARIN ADVENTURE - TUGAS SISWA*\n\n` +
                  `Halo semuanya! Silakan buka tautan Ruang Tugas berikut untuk mengerjakan:\n` +
                  `📌 *${taskName}*\n\n` +
                  `🔗 *Buka Halaman Tugas:* ${link}\n\n` +
                  `Selamat belajar, dengarkan audio pelan, dan kumpulkan telur dinosaurusnya! 🦕✨`;
...
function initDinoApp() {
  ...
  if (typeof DinoStoryReader !== 'undefined') {
    window.storyReaderApp = new DinoStoryReader();
  }
  ...
  function parseStudentTaskUrl() {
    ...
    if (brandSub) {
      brandSub.textContent = '🎒 Ruang Latihan Tugas Siswa SD 🌿';
    }
    ...
    if (mod === 'stroke') {
      ...
      if (bannerDesc) bannerDesc.textContent = `• Membaca Cerita Pelajaran Han Yu ${book} (Dengarkan Audio Pelan 🐢🗣️)`;

      const storyBookSel = document.getElementById('story-book-select');
      if (storyBookSel) storyBookSel.value = book;

      setTimeout(() => {
        if (window.storyReaderApp) {
          window.storyReaderApp.loadBookStory(book);
        }
      }, 100);
      ...
      if (bannerDesc) bannerDesc.textContent = `• Kuis Jumlah Guratan & Kosakata Han Yu (Soal Ber-Audio 🔊)`;
